...
for folder in range(2,6):
    source_path = "/home/justin/Desktop/Jupyter/Test Data/Image/Guantanamo/"+ str(folder) +"/"
    path="/home/justin/Desktop/GuantanamoFeatures/G_features_"+str(kernel_size)+"/"
    
    pathlib.Path(path+"G_features/"+str(folder)+"/").mkdir(parents=True, exist_ok=True)
    #pathlib.Path(path+"G_features_raw/"+str(folder)+"/").mkdir(parents=True, exist_ok=True)
    #pathlib.Path(path+"G_features_steps/"+str(folder)+"/").mkdir(parents=True, exist_ok=True)
    ...
    for filename in os.listdir(source_path):
        ...
        while True:
            ...
            if(X==-1 and Y==-1):
                ...
                if(flat_list.count(125) < flat_list_len*(1/2) and flat_list.count(0) > flat_list_len*(1/4)):
                    #Image.fromarray(features).save(path+"/G_features_raw/"+str(folder)+"/"+filename[:-4]+".png")
                    features = SnipFeature(thresh_copy,X,Y)
                    Image.fromarray(features).save(path+"/G_features/"+str(folder)+"/"+filename[:-4]+"-"+str(name)+"-G"+".png")
                    name+=1

                #img = Image.fromarray(thresh)
                #img.save(path+"/G_feature_steps/"+str(folder)+"/"+filename[:-4]+".png")
                #name_steps+=1
        #break
